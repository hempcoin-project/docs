# Hempcoin Hot + Cold wallet MasterNode setup guide
...
It's **Hot** because it's out on the public internet 24/7, directly accessible on the peer-to-peer port (TCP **27577
**), much more vulnerable than a **Cold** wallet. 
...
### Cold Wallet
On the other side, the **Cold** wallet (Windows, OSX, Linux) holds the THC collateral (**50,000** THC) and is used to enable the MasterNode server and collect rewards for its services.
...
This will run 24/7 and provide services to the network via TCP port **27577
** for which it will be rewarded with coins. It will run with an empty wallet reducing the risk of loosing the funds in the event of an attack.
...
If you are running the MasterNode server in Amazon AWS or if additional firewalls are in place, you need to allow incoming connections on port TCP **27577
** from any IP address.

### 5. Install the Hempcoin CLI wallet. Always download the latest [release available](https://github.com/Hempcoin-project/Hempcoin/releases), unpack it


Download and unpack the Hempcoin wallet binaries by running the following commands:

This command uses a short github url to download [Hempcoin-4.1.1-x86_64-linux.tar.gz](https://github.com/Hempcoin-project/Hempcoin/releases/download/4.1.1/Hempcoin-4.1.1-x86_64-linux.tar.gz) and unpack the binaries in the PATH:

```
wget -qO- https://git.io/fSGfK | sudo tar xvz -C /usr/local/bin/
Hempcoind
```

You'll get a start error like `Error: To use Hempcoind, or the -server option to Hempcoin-qt, you must set an rpcpassword in the configuration file`. It's expected because we haven't created the config file yet.

The service will only start for a second and create the initial data directory(`/root/.Hempcoin/`).

### 6. Edit the MasterNode main wallet configuration file:
```
nano /root/.Hempcoin/Hempcoin.conf
```
...
listen=1
daemon=1
externalip=<public_mn_ip_address_here>:27577

masternodeaddr=<public_mn_ip_address_here>:27577

addnode=seeds.THC.io
```
...
listen=1
daemon=1
externalip=199.247.10.25:27577

masternodeaddr=199.247.10.25:27577

addnode=seeds.THC.io
```

The IP address (`199.247.10.25` in this example) will be different for you. Use the `ifconfig` command to find out your IP address, normally the address of the `eth0` interface. We are going to use this IP and port (27577
) in the Cold Wallet setup(Step 2) as well.
...
```
hempcoin-cli masternode genkey
```
...
Add these two lines at the end of the file. The second line is taking the value you received from the `hempcoin-cli masternode genkey` command:
```
masternode=1
...
### 1. Install and open the Hempcoin-Qt wallet on your machine.

 * If you have a previous Hempcoin wallet installed, backup the `wallet.dat`, uninstall it then delete its original data directory.
 * Download the newest Hempcoin Qt wallet from: https://github.com/Hempcoin-project/Hempcoin/releases
...
### 3. Send EXACTLY 50000 THC coins to the address you just copied. Double check you've got the correct address before transferring the funds.
If you are sending from an exchange, make sure you account for the withdrawal fee so that you get EXACTLY EXACTLY EXACTLY 50000 THC in. This is a common error that will cause the next step to not give you the transaction id needed later on. 
For example, to withdraw from `Stocks.Exchange` the exact ammount for a MasterNode, you need to specify the ammount of **50000.001** to account for the fee.
 ...
 * Windows: `~\AppData\Roaming\Hempcoin`
...
```
MN1 199.247.10.25:27577
 87LBTcfgkepEddWNFrJcut76rFp9wQG6rgbqPhqHWGvy13A9hJK c19972e47d2a77d3ff23c2dbd8b2b204f9a64a46fed0608ce57cf76ba9216487 1
```
...
Where `87LBTcfgkepEddWNFrJcut76rFp9wQG6rgbqPhqHWGvy13A9hJK` is your masternode key from (Part 1), the value used for `masternodeprivkey` in `/root/.Hempcoin/Hempcoin.conf`.
...
You should now be able to see your MasterNode(s) **ENABLED** on this web page: [http://mn.THC.io](http://mn.THC.io)
